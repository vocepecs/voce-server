...
from models import PatientCsLogModel, UserModel, CsOutputImageModel, ComunicativeSessionModel, ContextModel
...
class ContextFrequenceGraph(Resource):
    ...
    def get(self):
        ...
        if len(image_id_count):
            ord_image_id_count = sorted(image_id_count, key=lambda x: x["count_distinct"], reverse=True)
    ...
    def generate_query(self, date_start, date_end, patient_id=None, centre_id=None, operator_id=None):
        ...
        context_id_count = []  # Output: Lista di dizionari con il count per ogni id e label
        if operator_id:
        ...
        for image in image_list:
            for context in image.image_context:
                if not any(context.id == context_count["context_id"] for context_count in context_id_count):
                    context_id_count.append({
                    ...
                    context_count = list(
                         filter(lambda x: x["context_id"] == context.id, context_id_count))[0]


                    if not any(image.id == image_count["image_id"] for image_count in context_count["image_list"]):
                        context_count["image_list"].append({
                            "image_id": image.id,
                            "label": image.label,
                            "count": 1
                        })
                        context_count["count_distinct"] = context_count["count_distinct"] +1
                    else:
                        image_count = list(filter(lambda x: x["image_id"] == image.id, context_count["image_list"]))[0]
