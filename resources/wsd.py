...
class WordSenseDisambiguation(Resource):

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    max_seq_length = 128
    gloss_selection_record = namedtuple("GlossSelectionRecord", [
                                        "guid", "sentence", "sense_keys", "glosses", "targets"])
    bert_input = namedtuple(
    ...
    def get_predictions(self, model, tokenizer, sentence, amb_word, pos):
        ...
        with torch.no_grad():
            ...
            logits = torch.zeros(len(definitions), dtype=torch.double).to(self.device)
            
            print(f"features len: {len(features)}")
            print(f"features: {features}")
            
            for i, bert_input in list(enumerate(features)):#tqdm(list(enumerate(features)), desc="Progress"):
                ...
                logits[i] = model.ranking_linear(
                    model.bert(
                        input_ids=torch.tensor(bert_input.input_ids, dtype=torch.long).unsqueeze(0).to(self.device),
                        attention_mask=torch.tensor(bert_input.input_mask, dtype=torch.long).unsqueeze(0).to(self.device),
                        token_type_ids=torch.tensor(bert_input.segment_ids, dtype=torch.long).unsqueeze(0).to(self.device)
                    )[1]
                )
    ...
    def post(self):
        ...
        phrase = request.args.get("phrase", default=None, type=str)
        sentence = (translator.translate(phrase, src="it", dest="en")).text.lower()
        ...
        for word in amb_word_list:
            predictions = self.get_predictions(
                model, tokenizer, sentence, word[0], word[1])
